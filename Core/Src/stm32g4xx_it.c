...
void can_tx_rx(void){
	...
	int no_message = HAL_FDCAN_GetRxMessage(&hfdcan1, FDCAN_RX_FIFO0, &can_rx.rx_header, can_rx.data);
	if(!no_message){
//		printf("%d\r\n",can_rx.rx_header.Identifier);
		pack_reply(&can_tx, CANID_SLAVE, 0x10);	// Pack response
		HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &can_tx.tx_header, can_tx.data);	// Send response
		  }
	}
